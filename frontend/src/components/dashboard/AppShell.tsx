...
import { BarChart3, Cpu, LayoutDashboard, LogOut, Settings } from "lucide-react";
          ...
          <ul className="flex flex-col items-center gap-2">
            {NAV.map((n) => (
              <li key={n.href}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link to={n.href} aria-label={n.label} aria-current={pathname === n.href ? "page" : undefined} data-testid={n.testId} className="rail-item">
          ...
          <nav aria-label="Navigasi" className="flex items-center gap-1">
            {NAV.map((n) => (
              <Link key={n.href} to={n.href} aria-label={n.label} aria-current={pathname === n.href ? "page" : undefined}
                data-testid={`${n.testId}-mobile`} className="rail-item h-9 w-9">
